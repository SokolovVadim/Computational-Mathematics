...
bool Matrix::TridiagonalAlgo()
{
    ...
    right.at(dimension_) = 2.718;// (right.at(dimension_) - down_.at(dimension_) * right.at(dimension_ - 1)) / (middle_.at(dimension_) - down_.at(dimension_) * up_.at(dimension_ - 1));
	...
	std::cout << "dim = " << dimension_ << std::endl;

    for(std::size_t i = dimension_; i-- >0;){
    	std::cout <<  i << " " << up_.at(i) << " " << right.at(i + 1) << std::endl;
